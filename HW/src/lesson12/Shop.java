...
class Cart{
	...
	public void addProduct(Products p, int num) {
		...
		System.out.println("Продукт " + product[count].getName() +" , ценой "+ product[count].getPrice() + " руб., в количестве " + numProduct[count]+ " шт., добавлен в корзину");
		count++;
	}
	public int getNumProduct(int i) {return numProduct[i];}
...
class Products{
	...
	public String toString() {
		return id + " - [" + name + ", производитель= " + manufacturer + ", рейтинг= " + rate + ", цена= " + price
				+ "]";
	}
...
public class Shop {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		int k=3;
		System.out.println("Категория " + categories[k-1].getName());
		System.out.println("Выберите товар:");
		for (Products p: categories[k-1].getProduct()) {
			System.out.println(p.toString());
		}
		k=15;
		System.out.println("Вы выбрали: " + products[k-1].getName()+ ", сколько штук положить в корзину?");
		int p1=14;
		int n1=2;
		int p2=6;
		int n2=3;
	...
	public static void checkLogin(User user) {
		...
		if (scan1.hasNextLine() && scan2.hasNextLine()) {
			...
			else {System.out.println("Логин и пароль не соответсвуют");}
		}
	}
}
